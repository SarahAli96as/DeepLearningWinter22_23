...
class Classifier(nn.Module, ABC):
    ...
    def predict_proba_scores(self, z: Tensor) -> Tensor:
        ...
        # ====== YOUR CODE: ======
        scores_sum = torch.reshape(torch.sum(z, dim=1), (z.shape[0], -1)) ## Shape (N, 1)
        z_normalized = z/scores_sum ## score_sum -> (N,C) with broadcasting
...
class BinaryClassifier(Classifier):
    ...
    def _classify(self, y_proba: Tensor):
        ...
        #  Output should be a (N,) integer tensor.
        # ====== YOUR CODE: ======
        y_pred = torch.zeros(y_proba.shape[0],dtype=torch.int32)
        y_pred[y_proba[:,self.positive_class] > self.threshold] = 1
        return y_pred
        # ========================
...
def plot_decision_boundary_2d(
    ...
    # ====== YOUR CODE: ======
    x1_max, x1_min = torch.max(x[:, 0]), torch.min(x[:, 0])
    x2_max, x2_min = torch.max(x[:, 1]), torch.min(x[:, 1])

    
    x1 = torch.arange(x1_min, x1_max, dx)
    x2 = torch.arange(x2_min, x2_max, dx)
    
    x1_grid, x2_grid = torch.meshgrid(x1, x2)
    
    x1_grid_flat = torch.reshape(x1_grid, (-1, 1))
    x2_grid_flat = torch.reshape(x2_grid, (-1, 1))
    
    x12 = torch.hstack((x1_grid_flat, x2_grid_flat))

    y_hat = classifier.classify(x12).reshape(x1_grid.shape)
...
def select_roc_thresh(
    ...
    # ====== YOUR CODE: ======
    y_scores = classifier.forward(x)
    y_scores_positive_class = y_scores[:, classifier.positive_class]

    fpr, tpr, thresholds = roc_curve(y.numpy(), y_scores_positive_class.detach().numpy(), pos_label = classifier.positive_class)

    
    ##Ideally we want fpr=0 and tpr=1, so let's find the min distance of (fpr,tpr) from (0,1)
    dist_from_ideal = torch.sqrt(torch.tensor(fpr)**2 + (1-torch.tensor(tpr))**2)
    _, optimal_thresh_idx = torch.min(dist_from_ideal.reshape(1,-1), 1)
    
    optimal_thresh_idx = optimal_thresh_idx.item()
