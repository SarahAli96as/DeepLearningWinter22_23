...
class CNN(nn.Module):
    ...
    def _make_feature_extractor(self):
        ...
        for channel_idx in range(len(self.channels)):
            ...
            if channel_idx == 0:
                ...
                in_channels_layer = self.channels[channel_idx-1]

            layers.append(
                nn.Conv2d( in_channels = in_channels_layer,
            ...
            #Pool
            if (channel_idx + 1) % self.pool_every == 0:
    ...
    def _n_features(self) -> int:
        ...
        try:
            # ====== YOUR CODE: ======
            input_init = torch.zeros(self.in_size)
            input_init = torch.unsqueeze(input_init, 0)
            ext = self.feature_extractor(input_init)
            n_features = torch.numel(ext)
            return n_features
    ...
    def _make_mlp(self):
        ...
        active_funcs = ACTIVATIONS[self.activation_type](**self.activation_params)
        mlp = MLP(
                in_dim = self._n_features(),
                dims = self.hidden_dims+[self.out_classes],
                nonlins = [*[active_funcs] * (len(self.hidden_dims)), 'none']
                )
        # ========================
        return mlp
