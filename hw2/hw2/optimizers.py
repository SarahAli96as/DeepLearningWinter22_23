...
class MomentumSGD(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, momentum=0.9):
        ...
        for _ , dp in params:
            if dp is None:
                ...
                self.vp.append(torch.zeros_like(dp))
        # ========================
    ...
    def step(self):
        for i , (p, dp) in enumerate(self.params):
            ...
            # ====== YOUR CODE: ======
            dp += self.reg * p
            self.vp[i] = self.momentum * self.vp[i] + dp
            p += - (self.learn_rate * self.vp[i])
            # ========================
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        for _, dp in params:
            if dp is None:
                self.grad_sq.append(0)
            else:
                self.grad_sq.append(torch.zeros_like(dp))
        # ========================
    ...
    def step(self):
        for i, (p, dp) in enumerate(self.params):
            ...
            # ====== YOUR CODE: ======
            dp += self.reg * p
            self.grad_sq[i] =  self.decay * self.grad_sq[i] + ( 1 - self.decay) * dp * dp
            p += - (self.learn_rate * dp / (torch.sqrt(self.grad_sq[i]) + self.eps))
            # ========================
