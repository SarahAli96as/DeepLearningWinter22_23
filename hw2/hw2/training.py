...
class LayerTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        y_scores = self.model.forward(X.reshape(X.shape[0], -1)) #Flatten the features from col 1 - 3*32*32 
        loss = self.loss_fn.forward(y_scores, y)
        self.optimizer.zero_grad()
        self.model.backward(self.loss_fn.backward())
        self.optimizer.step()

        correct_score = torch.argmax(y_scores, dim=1)
        num_correct = torch.sum(correct_score == y).item()
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        y_scores = self.model.forward(X.reshape(X.shape[0], -1))
        loss = self.loss_fn.forward(y_scores, y)
